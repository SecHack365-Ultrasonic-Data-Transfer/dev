...
### 3wayハンドシェイク(の初期値決め)
def make_3way_hand_shake() -> str :
    return random.randint(init_min, init_max)


### Seq No.(16 -> 8bit)
def update_seq_no(recv_seq: int, recv_ack: int) -> int :
    if recv_seq == -1 :     #3wayハンドシェイク開始時
        return format(make_3way_hand_shake(), "b").zfill(seq_ack_len)
    else :
...
def main(my_code, oth_code, recv_seq, recv_ack, recv_data_len, flag):
    ...
    send_bin +=  padding
    print(send_bin)
    send_bin = format(int(send_bin, 2),"x")
    print(send_bin)
    return send_bin, sender_code, recver_code, send_seq, send_ack, send_flag, padding